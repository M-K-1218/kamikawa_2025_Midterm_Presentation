...
\presentationdate{2025}{10}{3} % 発表会の日付
...
\begin{figure}[tb]
  ...
  \caption{シミュレーション結果}
  \vspace{-7mm} % キャプション下の余白を削る
\end{figure}
今回のケースでは4台のAPを四隅に等間隔に配置した．このAPのWi-Fi規格は802.11ax-2.4Gtbzとし，チャネルはそれぞれ別のものを1つずつを割り当てた．
それぞれのAPには異なるチャネル使用率を設定し，環境における混雑度の違いを再現した．今回の設定はAP0=0.2, AP1=0.3, AP2=0.4, AP3=0.5である．
...
\begin{figure}[tb]
 ...
 \caption{{移動距離の分布}}
  \vspace{-6mm} % キャプション下の余白を削る
\end{figure}
また，移動距離の観点では，多くのユーザが5m未満の小規模な移動で十分な改善を得られており，過度な行動負担を課すことなく全体スループットが改善した．
